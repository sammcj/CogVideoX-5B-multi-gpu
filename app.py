...
def load_model(rank, world_size):
    ...
    i2v_transformer = DDP(i2v_transformer, device_ids=[rank])

    pipe_video = CogVideoXVideoToVideoPipeline.from_pretrained(
        "THUDM/CogVideoX-5b",
        transformer=pipe.module.transformer,
        vae=pipe.module.vae,
        scheduler=pipe.module.scheduler,
        tokenizer=pipe.module.tokenizer,
        text_encoder=pipe.module.text_encoder,
        torch_dtype=torch.bfloat16,
    ).to(rank)
    pipe_video = DDP(pipe_video, device_ids=[rank])

    pipe_image = CogVideoXImageToVideoPipeline.from_pretrained(
        "THUDM/CogVideoX-5b-I2V",
        transformer=i2v_transformer.module,
        vae=pipe.module.vae,
        scheduler=pipe.module.scheduler,
        tokenizer=pipe.module.tokenizer,
        text_encoder=pipe.module.text_encoder,
    ...
    pipe_image = DDP(pipe_image, device_ids=[rank])

    return pipe, i2v_transformer, pipe_video, pipe_image
...
def infer(
    ...
    pipe, i2v_transformer, pipe_video, pipe_image = load_model(rank, world_size)
    ...
    if video_input is not None:
        ...
        image_input = Image.fromarray(image_input).resize(
            size=(720, 480)
        )  # Convert to PIL
        image = load_image(image_input)
        video_pt = pipe_image.module(
            image=image,
            prompt=prompt,
            num_inference_steps=num_inference_steps,
            num_videos_per_prompt=1,
...
def run_parallel(
    ...
    return output[0]  # Return result from the first process
...
with gr.Blocks() as demo:
    ...
    def generate(
        ...
        if scale_status:
            video_pt = utils.upscale_batch_and_concatenate(
                upscale_model, video_pt, device
            )
        if rife_status:
            video_pt = rife_inference_with_latents(frame_interpolation_model, video_pt)

        video_path = utils.save_video(video_pt, fps=8)
        gif_path = utils.convert_to_gif(video_path)

        return (
            video_path,
            gr.update(visible=True, value=video_path),
            gr.update(visible=True, value=gif_path),
            gr.update(visible=True, value=seed),
        )
